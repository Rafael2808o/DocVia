...
const typeVisuals = {
  contrato: { color: '#62D4C7', background: '#62D4C71F', Icon: FileText },
  exame: { color: '#24D6AF', background: '#24D6AF1F', Icon: HeartPulse },
  boleto: { color: '#E5BD3E', background: '#E5BD3E1F', Icon: ReceiptText },
  outro: { color: '#62D4C7', background: '#62D4C71F', Icon: FileText },
};
const searchFocusStyles = StyleSheet.create({ active: { borderColor: '#147D92', boxShadow: '0 0 10px rgba(20, 125, 146, 0.30)', shadowColor: '#147D92', shadowOpacity: .28, shadowRadius: 10, shadowOffset: { width: 0, height: 0 }, elevation: 4 } });
...
function statusFor(document) {
  ...
  if (document.status !== 'done') return { label: 'Processamento em andamento', color: '#62D4C7' };
...
function EmptyDocuments({ hasDocuments, onUpload }) {
  ...
  const content = <><View style={styles.emptyIcon}><Upload size={20} color="#7FD9D0" strokeWidth={1.9} /></View><View style={styles.emptyCopy}><Text style={styles.emptyTitle}>{title}</Text><Text style={styles.emptyText}>{text}</Text></View><ChevronRight size={17} color="#646878" /></>;
