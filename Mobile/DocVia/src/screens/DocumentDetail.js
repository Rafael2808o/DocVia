...
function warningTone(item) {
  ...
  const conditionalRule = /\b(se|caso|em caso|poder[aá]|ser[aá] aplicada|ap[oó]s)\b/.test(text);
  if (['critico', 'crítico', 'critical', 'alta', 'high'].includes(priority) && !conditionalRule) return { key: 'Critical', label: 'CRÍTICO', color: '#FF737D' };
...
export default function DocumentDetail({ id, back }) {
  ...
  const obligations = uniqueInsights((doc.analysis_structured?.obligations || []).map(item => item.description || item.descricao || item));
  const status = doc.status === 'done' ? 'PRONTO' : doc.status === 'failed' ? 'ERRO' : 'ANALISANDO';
  const dueDate = boletoDueDate(doc, boleto);
  const normalizedCosts = normalizeCosts(doc.analysis_costs || [], doc.extracted_text || '', doc.analysis_structured?.financial_items || []);
  const costGroups = [
    ['Valores', normalizedCosts.filter((item) => !['credit', 'rule'].includes(item.category))],
    ['Créditos e abatimentos', normalizedCosts.filter((item) => item.category === 'credit')],
    ['Regras financeiras', normalizedCosts.filter((item) => item.category === 'rule')],
  ...
  const content = () => {
    if (tab === 'Resumo') return <><Text style={styles.summary}>{doc.analysis_summary || 'A análise ainda está sendo processada. Volte em alguns instantes.'}</Text>{obligations.length ? <><Text style={styles.sectionTitle}>Obrigações identificadas</Text>{obligations.map((item, index) => <DetailCard key={`obligation-${index}`} style={styles.actionCard}><Text style={styles.actionText}>{typeof item === 'string' ? item : item.descricao || item.description}</Text></DetailCard>)}</> : null}<Text style={styles.sectionTitle}>Ações recomendadas</Text>{actionItems.length ? actionItems.map((item, index) => <DetailCard key={index} style={styles.actionCard}><Text style={styles.actionText}>{typeof item === 'string' ? item : item.descricao || item.description}</Text></DetailCard>) : <Text style={styles.emptyText}>Nenhuma ação recomendada.</Text>}</>;
    if (tab === 'Prazos') return normalizedDeadlines.length ? normalizedDeadlines.map((item, index) => { const due = deadlineDate(item); const recurring = item.recorrencia === 'mensal'; return <DetailCard key={`${item.descricao}-${due}-${index}`} style={styles.dataCard}><View style={styles.dataCopy}><Text style={styles.dataTitle}>{item.descricao}</Text>{due ? <Text style={styles.dataHint}>{date(due)}{recurring ? ' · recorrência mensal' : ''}{item.date_origin === 'DERIVED' ? ' · próxima ocorrência calculada' : ''}</Text> : null}</View>{due ? <Pressable onPress={() => reminder(due)} style={styles.smallButton}><Text style={styles.smallButtonText}>Lembrar</Text></Pressable> : null}</DetailCard>; }) : <Text style={styles.emptyText}>Nenhum prazo identificado.</Text>;
    if (tab === 'Custos') return normalizedCosts.length ? costGroups.map(([label, entries]) => <View key={label}><Text style={styles.costGroupTitle}>{label}</Text>{entries.map((item, index) => <DetailCard key={`${item.description}-${item.amount}-${index}`} style={styles.dataCard}><View style={styles.dataCopy}><Text style={styles.dataTitle}>{item.description}</Text>{item.type === 'LINE_ITEM' ? <><Text style={styles.dataHint}>{item.quantity} {item.quantity_unit === 'TERABYTE' ? 'TB' : item.quantity_unit === 'GIGABYTE' ? 'GB' : item.quantity_unit === 'USER' ? 'usuários' : item.quantity_unit === 'HOUR' ? 'horas' : 'unidades'} × {formatBrl(item.unit_price)}{item.price_unit === 'TERABYTE' ? '/TB' : item.price_unit === 'GIGABYTE' ? '/GB' : item.price_unit === 'USER' ? '/usuário' : item.price_unit === 'HOUR' ? '/hora' : ''}</Text><Text style={styles.subtotal}>Subtotal: {formatBrl(item.subtotal)}</Text>{item.validation?.status === 'MISMATCH' ? <Text style={styles.ruleHint}>O subtotal não confere com a multiplicação.</Text> : null}</> : item.amount ? <Text style={styles.dataHint}>{item.amount}</Text> : null}{item.conditional ? <Text style={styles.ruleHint}>Regra condicional — não é cobrança atual</Text> : null}</View></DetailCard>)}</View>) : <Text style={styles.emptyText}>Nenhum custo identificado.</Text>;
    ...
    return <DetailCard><Text selectable style={styles.summary}>{String(doc.extracted_text || 'O texto extraído ficará disponível quando o processamento terminar.').replace(/\[\[(?:\/?TABLE|PAGE)(?:\s+[^\]]*)?\]\]/gi, '')}</Text><Pressable onPress={async () => { await Clipboard.setStringAsync(String(doc.extracted_text || '').replace(/\[\[(?:\/?TABLE|PAGE)(?:\s+[^\]]*)?\]\]/gi, '')); Alert.alert('Texto copiado'); }} style={styles.copyButton}><Copy size={15} color="#91E0D8" /><Text style={styles.copyText}>Copiar texto</Text></Pressable></DetailCard>;
  };
...
const styles = StyleSheet.create({
  ...
  costGroupTitle: { color: '#91E0D8', fontSize: 11, fontWeight: '900', letterSpacing: .7, marginTop: 4, marginBottom: 8 }, subtotal: { color: '#ECECF3', fontSize: 12, fontWeight: '800', marginTop: 6 }, ruleHint: { color: '#C9A74C', fontSize: 9, marginTop: 5 },
