...
export default function PrivacyPolicy({ back }) {
  ...
  return <ScrollView style={common.screen} contentContainerStyle={styles.content}>
    ...
    <View style={styles.heading}><View style={styles.icon}><ShieldCheck size={22} color="#A99DFF" /></View><View><Text style={styles.title}>Privacidade</Text><Text style={styles.version}>Versão {policy.version} · atualizada em {policy.updated_at}</Text></View></View>
    ...
    {policy.account_deletion_url ? <Pressable accessibilityRole="link" onPress={() => Linking.openURL(policy.account_deletion_url)} style={styles.link}><ExternalLink size={15} color="#A99DFF" /><Text style={styles.linkText}>Solicitar exclusão pela web</Text></Pressable> : null}
  </ScrollView>;
}

const styles = StyleSheet.create({
  content: { paddingHorizontal: 28, paddingTop: 58, paddingBottom: 60, gap: 16 }, back: { flexDirection: 'row', alignItems: 'center', gap: 6, alignSelf: 'flex-start', paddingVertical: 8 }, backText: { color: '#B7AEFF', fontSize: 12, fontWeight: '800' }, heading: { flexDirection: 'row', alignItems: 'center', gap: 12 }, icon: { width: 46, height: 46, borderRadius: 16, alignItems: 'center', justifyContent: 'center', backgroundColor: '#171433', borderWidth: 1, borderColor: '#38307E' }, title: { color: '#F4F4F8', fontSize: 24, fontWeight: '800' }, version: { color: '#858796', fontSize: 10, marginTop: 4 }, summary: { color: '#D7D7E0', fontSize: 13, lineHeight: 21 }, label: { color: '#888A99', fontSize: 9, fontWeight: '900', letterSpacing: 1.2, marginTop: 8 }, card: { borderRadius: 17, borderWidth: 1, borderColor: '#292B35', backgroundColor: '#111219', padding: 15, gap: 8 }, item: { color: '#D7D7E0', fontSize: 12, lineHeight: 18 }, body: { color: '#A0A2B0', fontSize: 12, lineHeight: 19 }, warning: { color: '#FFC52E', fontSize: 11, lineHeight: 17 }, link: { minHeight: 44, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 7 }, linkText: { color: '#B7AEFF', fontSize: 12, fontWeight: '800' },
});
