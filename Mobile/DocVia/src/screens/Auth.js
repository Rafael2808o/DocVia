...
export function Login({ login, go }) {
  ...
  return <AuthLayout style={{ paddingTop: insets.top + 42, paddingBottom: Math.max(insets.bottom, 24) }}>
    ...
    <View style={styles.loginForm}>
      <Field label="E-MAIL" icon={(color) => <Mail size={17} color={color} strokeWidth={1.8} />}><TextInput value={email} onChangeText={setEmail} placeholder="seuemail@exemplo.com" placeholderTextColor="#8B899A" selectionColor={accent} autoComplete="off" importantForAutofill="no" keyboardType="email-address" autoCapitalize="none" autoCorrect={false} style={styles.authInput} accessibilityLabel="E-mail" /></Field>
      <Field label="SENHA" icon={(color) => <LockKeyhole size={17} color={color} strokeWidth={1.8} />}><TextInput value={senha} onChangeText={setSenha} placeholder="••••••••••" placeholderTextColor="#B7B5C5" selectionColor={accent} autoComplete="off" importantForAutofill="no" secureTextEntry={!show} style={styles.authInput} accessibilityLabel="Senha" /><Pressable accessibilityRole="button" accessibilityLabel={show ? 'Ocultar senha' : 'Mostrar senha'} onPress={() => setShow((value) => !value)} hitSlop={10}>{show ? <EyeOff size={17} color={darkIcon} /> : <Eye size={17} color={darkIcon} />}</Pressable></Field>
...
export function Register({ login, go }) {
  ...
  return <AuthLayout style={{ paddingTop: insets.top + 30, paddingBottom: Math.max(insets.bottom, 24) }}>
    ...
    <View style={styles.registerForm}>
      <Field label="NOME" icon={(color) => <UserRound size={17} color={color} strokeWidth={1.8} />}><TextInput value={name} onChangeText={setName} placeholder="Seu nome completo" placeholderTextColor="#8B899A" selectionColor={accent} autoComplete="off" importantForAutofill="no" autoCapitalize="words" style={styles.authInput} accessibilityLabel="Nome completo" /></Field>
      <Field label="E-MAIL" icon={(color) => <Mail size={17} color={color} strokeWidth={1.8} />}><TextInput value={email} onChangeText={setEmail} placeholder="seuemail@exemplo.com" placeholderTextColor="#8B899A" selectionColor={accent} autoComplete="off" importantForAutofill="no" keyboardType="email-address" autoCapitalize="none" autoCorrect={false} style={styles.authInput} accessibilityLabel="E-mail" /></Field>
      <Field label="SENHA" icon={(color) => <LockKeyhole size={17} color={color} strokeWidth={1.8} />}><TextInput value={senha} onChangeText={setSenha} placeholder="••••••••••" placeholderTextColor="#B7B5C5" selectionColor={accent} autoComplete="off" importantForAutofill="no" secureTextEntry={!show} style={styles.authInput} accessibilityLabel="Senha" /><Pressable accessibilityRole="button" accessibilityLabel={show ? 'Ocultar senha' : 'Mostrar senha'} onPress={() => setShow((value) => !value)} hitSlop={10}>{show ? <EyeOff size={17} color={darkIcon} /> : <Eye size={17} color={darkIcon} />}</Pressable></Field>
...
export function Forgot({ go }) {
  ...
  return <AuthLayout style={{ paddingTop: insets.top + 30, paddingBottom: Math.max(insets.bottom, 24) }}>
    ...
    <View style={styles.forgotForm}><Field label="E-MAIL" icon={(color) => <Mail size={17} color={color} strokeWidth={1.8} />}><TextInput value={email} onChangeText={setEmail} placeholder="seuemail@exemplo.com" placeholderTextColor="#8B899A" selectionColor={accent} autoComplete="off" importantForAutofill="no" keyboardType="email-address" autoCapitalize="none" autoCorrect={false} style={styles.authInput} accessibilityLabel="E-mail" /></Field>{message ? <Text style={styles.message}>{message}</Text> : null}<AuthButton title="Enviar instruções" loading={loading} onPress={submit} /></View>
