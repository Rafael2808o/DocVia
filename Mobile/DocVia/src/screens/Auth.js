...
const accent = '#147D92';
const darkIcon = '#71868B';

function Field({ label, icon, children }) {
  const focused = useSharedValue(0);
  const focusStyle = useAnimatedStyle(() => ({ borderColor: interpolateColor(focused.value, [0, 1], ['#25383D', accent]), shadowColor: accent, shadowOpacity: focused.value * .28, shadowRadius: focused.value * 10, elevation: focused.value * 4 }));
...
export function Login({ login, go, onVerificationRequired }) {
  ...
  const submit = async () => { if (!email || !senha) return setError('Informe seu e-mail e senha.'); setLoading(true); try { await login(email, senha); } catch (e) { if (e.code === 'EMAIL_NOT_VERIFIED') onVerificationRequired(email.trim().toLowerCase()); else setError(e.message); } finally { setLoading(false); } };
...
export function VerifyEmail({ go, email = '', initialToken = '' }) {
  const [message, setMessage] = useState(initialToken ? 'Confirmando seu e-mail...' : 'Enviamos um link de confirmação para o seu e-mail.');
  ...
  useEffect(() => {
    ...
    let active = true;
    authApi.verifyEmail(initialToken)
      .then((result) => { if (active) { setVerified(true); setMessage(result.message); } })
  ...
  const resend = async () => {
    if (!email) return setMessage('Volte ao login, informe seu e-mail e tente entrar para reenviar.');
    setLoading(true);
    try { const result = await authApi.resendVerification(email); setMessage(result.message); }
    catch (error) { setMessage(error.message); }
    finally { setLoading(false); }
  };

  return <Screen style={styles.legacy}>
    <Text style={common.title}>{verified ? 'E-mail confirmado' : 'Confirme seu e-mail'}</Text>
    <Text style={common.subtitle}>{email ? `Enviamos a confirmação para ${email}.` : 'Abra o link recebido para ativar sua conta.'}</Text>
    <Text style={styles.message}>{message}</Text>
    {!verified && !initialToken ? <Button title={loading ? 'Aguarde...' : 'Reenviar confirmação'} disabled={loading} onPress={resend} /> : null}
    <Button title={verified ? 'Entrar' : 'Voltar ao login'} variant={verified ? 'primary' : 'ghost'} onPress={() => go('login')} />
  </Screen>;
}
...
const styles = StyleSheet.create({
  legacy: { justifyContent: 'center', flexGrow: 1, gap: spacing.md }, keyboard: { flex: 1 }, screen: { flexGrow: 1, paddingHorizontal: 39, backgroundColor: '#071316' }, brand: { color: '#62D4C7', fontSize: 11, fontWeight: '900', letterSpacing: 3, marginBottom: 20 }, title: { color: '#F4F7F7', fontSize: 29, lineHeight: 36, letterSpacing: .3, fontWeight: '800' }, subtitle: { color: '#AAB9BB', fontSize: 13, marginTop: 11 }, fieldLabel: { color: '#71868B', fontSize: 9, fontWeight: '800', letterSpacing: 1.6, marginTop: 5 }, authField: { height: 53, borderRadius: 20, borderWidth: 1, borderColor: '#25383D', backgroundColor: '#0E1B1E', flexDirection: 'row', alignItems: 'center', paddingHorizontal: 17, gap: 13 }, authFieldFocused: { borderColor: accent, shadowColor: accent, shadowOpacity: .28, shadowRadius: 10, shadowOffset: { width: 0, height: 0 }, elevation: 4 }, authInput: { flex: 1, color: '#F4F7F7', fontSize: 13, paddingVertical: 0, backgroundColor: '#0E1B1E' }, loginForm: { marginTop: 39, gap: 10 }, registerForm: { marginTop: 38, gap: 10 }, forgotForm: { marginTop: 38, gap: 10 }, authButton: { height: 55, marginTop: 11, borderRadius: 17, backgroundColor: accent, alignItems: 'center', justifyContent: 'center' }, authPressed: { opacity: .84, transform: [{ scale: .99 }] }, authDisabled: { opacity: .5 }, authButtonText: { color: '#FFFFFF', fontSize: 14, fontWeight: '800' }, error: { color: '#EF7373', fontSize: 11, lineHeight: 16 }, message: { color: '#AAB9BB', fontSize: 12, textAlign: 'center', lineHeight: 18 }, forgot: { height: 43, alignItems: 'center', justifyContent: 'center' }, linkText: { color: '#AAB9BB', fontSize: 13 }, loginBottom: { marginTop: 'auto', alignItems: 'center', gap: 15 }, question: { color: '#71868B', fontSize: 13 }, createButton: { width: '100%', height: 57, borderRadius: 17, borderWidth: 1, borderColor: '#26373B', alignItems: 'center', justifyContent: 'center' }, createText: { color: '#F4F7F7', fontSize: 14, fontWeight: '800' }, close: { width: 38, height: 38, borderRadius: 19, borderWidth: 1, borderColor: '#26373B', backgroundColor: '#0E1B1E', alignItems: 'center', justifyContent: 'center', marginBottom: 38 }, closePressed: { opacity: .72, transform: [{ scale: .96 }] }, loginLink: { alignSelf: 'center', height: 44, marginTop: 18, alignItems: 'center', justifyContent: 'center' }, linkPressed: { opacity: .65 }
});
