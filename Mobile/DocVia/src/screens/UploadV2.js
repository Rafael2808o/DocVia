...
function SourceButton({ Icon, label, active, onPress }) {
  return <Pressable accessibilityRole="button" accessibilityState={{ selected: active }} onPress={onPress} style={({ pressed }) => [styles.source, active && styles.sourceActive, pressed && styles.pressed]}><Icon size={18} color={active ? '#7FD9D0' : '#989AAA'} strokeWidth={1.8} /><Text style={[styles.sourceText, active && styles.sourceTextActive]}>{label}</Text></Pressable>;
}

function ProgressRing({ progress }) {
  const progressOffset = ringCircumference * (1 - progress / 100);
  return <View style={styles.progressSymbol}><Svg width={58} height={58} viewBox="0 0 58 58" style={styles.progressRing}><Circle cx="29" cy="29" r="24" fill="none" stroke="#19383D" strokeWidth="4" /><Circle cx="29" cy="29" r="24" fill="none" stroke="#62D4C7" strokeWidth="4" strokeLinecap="round" strokeDasharray={ringCircumference} strokeDashoffset={progressOffset} transform="rotate(-90 29 29)" /></Svg><Text style={styles.progressPercent}>{progress}%</Text></View>;
}
...
export default function UploadV2({ navigate }) {
  ...
  return <ScrollView style={common.screen} contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
    ...
    {source === 'text' ? <View style={styles.textBox}><View style={styles.textBoxHead}><View style={styles.uploadIcon}><TextCursorInput size={23} color="#62D4C7" strokeWidth={1.7} /></View><View><Text style={styles.dropTitle}>Cole ou digite o texto</Text><Text style={styles.textHint}>A IA vai analisar o conteúdo enviado.</Text></View></View><TextInput value={manualText} onChangeText={setManualText} multiline textAlignVertical="top" placeholder="Cole aqui o conteúdo do documento..." placeholderTextColor="#6F7180" selectionColor="#147D92" style={styles.textInput} accessibilityLabel="Texto do documento" /></View> : <Pressable accessibilityRole="button" accessibilityLabel="Escolher um arquivo" onPress={() => pick('file')} style={({ pressed }) => [styles.drop, pressed && styles.pressed]}>
      ...
      <View style={styles.dropCopy}><View style={styles.uploadIcon}><Upload size={24} color="#62D4C7" strokeWidth={1.7} /></View>
...
const styles = StyleSheet.create({
  ...
  dropCopy: { alignItems: 'center', transform: [{ translateY: -13 }] }, uploadIcon: { width: 58, height: 58, borderRadius: 17, backgroundColor: '#0B2429', borderWidth: 1, borderColor: '#1D5560', alignItems: 'center', justifyContent: 'center', marginBottom: 15 },
  ...
  sourceActive: { borderColor: '#246A76', backgroundColor: '#0B2429' },
  sourceText: { color: '#A1A3B0', fontSize: 11, fontWeight: '600' },
  sourceTextActive: { color: '#A5E7E1' },
  ...
  chipOn: { backgroundColor: '#147D92', borderColor: '#147D92' }, chipText: { color: '#A1A3B0', fontSize: 11, fontWeight: '600' }, chipTextOn: { color: '#FFFFFF' },
  consent: { flexDirection: 'row', alignItems: 'center', gap: 12, marginTop: 21, minHeight: 43 },
  checkbox: { width: 21, height: 21, borderRadius: 11, borderWidth: 1, borderColor: '#6F7280', alignItems: 'center', justifyContent: 'center' }, checkboxOn: { borderColor: '#147D92', backgroundColor: '#147D92' }, check: { color: '#FFFFFF', fontSize: 13, fontWeight: '900' },
  consentText: { flex: 1, color: '#A0A2AF', fontSize: 11, lineHeight: 16 }, policy: { color: '#62D4C7', fontWeight: '700' },
  send: { height: 56, borderRadius: 16, backgroundColor: '#147D92', alignItems: 'center', justifyContent: 'center', marginTop: 18 }, sendDisabled: { backgroundColor: '#181920' }, sendText: { color: '#FFFFFF', fontSize: 14, fontWeight: '800' }, sendTextDisabled: { color: '#757786' }, pressed: { opacity: .82, transform: [{ scale: .99 }] },
  resultContent: { paddingHorizontal: 28, paddingTop: 68, paddingBottom: 130 }, resultIntro: { alignItems: 'center', paddingHorizontal: 12 }, resultIcon: { width: 62, height: 62, borderRadius: 21, backgroundColor: '#0C282E', borderWidth: 1, borderColor: '#1F5964', alignItems: 'center', justifyContent: 'center', marginBottom: 20 }, resultIconError: { backgroundColor: '#291319', borderColor: '#71303A' }, resultEyebrow: { color: '#73D4CB', fontSize: 9, letterSpacing: 1.4, fontWeight: '800' }, resultTitle: { color: '#F4F4F8', fontSize: 23, fontWeight: '800', marginTop: 9 }, resultSubtitle: { color: '#9294A2', fontSize: 12, lineHeight: 18, textAlign: 'center', marginTop: 9 }, progressCard: { marginTop: 30, borderRadius: 19, borderWidth: 1, borderColor: '#282A35', backgroundColor: '#111219', paddingHorizontal: 19, paddingVertical: 19, gap: 15 }, progressCardError: { borderColor: '#54262F' }, progressStep: { minHeight: 22, flexDirection: 'row', alignItems: 'center' }, progressMarker: { width: 26, height: 33, alignItems: 'center', position: 'relative' }, progressLine: { position: 'absolute', top: 21, width: 2, height: 19, backgroundColor: '#30323E' }, progressLineDone: { backgroundColor: '#147D92' }, progressDot: { width: 21, height: 21, borderRadius: 11, backgroundColor: '#252631', borderWidth: 1, borderColor: '#3A3B47', alignItems: 'center', justifyContent: 'center' }, progressDotDone: { backgroundColor: '#147D92', borderColor: '#147D92' }, progressDotActive: { borderColor: '#74D8CE', boxShadow: '0 0 8px rgba(20, 125, 146, 0.38)' }, progressDotError: { backgroundColor: '#EF4444', borderColor: '#EF4444' }, progressText: { color: '#8F919E', fontSize: 12, marginLeft: 9 }, progressTextDone: { color: '#DDEDEA', fontWeight: '700' }, progressTextActive: { color: '#AFEAE4', fontWeight: '800' }, resultButton: { height: 54, borderRadius: 16, backgroundColor: '#147D92', alignItems: 'center', justifyContent: 'center', marginTop: 21 }, resultButtonText: { color: '#FFFFFF', fontSize: 14, fontWeight: '800' }
  , progressSymbol: { width: 58, height: 58, alignItems: 'center', justifyContent: 'center' }, progressRing: { position: 'absolute' }, progressPercent: { color: '#D6ECE8', fontSize: 13, fontWeight: '900' }
});
