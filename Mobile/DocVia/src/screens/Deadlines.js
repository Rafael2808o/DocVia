...
function DeadlineCard({ item, index }) {
  const accent = index % 2 ? '#8B80FF' : '#FFC52E';
  const days = remainingDays(item.due_date);
  const badgeColor = days <= 7 ? '#FFC52E' : '#8B80FF';
...
const styles = StyleSheet.create({
  ...
  calendar: { flexGrow: 1, justifyContent: 'center', gap: 7, paddingTop: 11, paddingBottom: 27 }, day: { width: 42, height: 70, borderRadius: 21, borderWidth: 1, borderColor: '#282A35', backgroundColor: '#111219', alignItems: 'center', justifyContent: 'center', gap: 6 }, dayActive: { backgroundColor: violet, borderColor: violet }, dayPressed: { opacity: .82, transform: [{ scale: .97 }] }, dayName: { color: '#878998', fontSize: 9, fontWeight: '700' }, dayNameActive: { color: '#E4E0FF' }, dayNumber: { color: '#F0F0F5', fontSize: 14, fontWeight: '800' }, dayNumberActive: { color: '#FFFFFF' },
