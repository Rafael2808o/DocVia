...
const documentColors = {
  contract: { color: '#62D4C7', background: '#62D4C71F', Icon: FileText },
...
export default function Home({ user, navigate, openDocument }) {
  ...
  return <ScrollView style={common.screen} contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor={primary} />}>
    ...
    <Pressable accessibilityRole="button" onPress={() => navigate('upload')} style={({ pressed }) => [styles.analyze, pressed && styles.analyzePressed]}><LinearGradient colors={['#0B5260', '#147D92', '#2BA99E']} locations={[0, .54, 1]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.analyzeGradient}><Sparkles size={24} color="#DDF5F1" strokeWidth={1.8} /><View style={styles.uploadCircle}><Upload size={17} color="#FFFFFF" strokeWidth={2} /></View><Text style={styles.analyzeTitle}>Analisar novo documento</Text><Text style={styles.analyzeText}>PDF, JPG ou PNG · até 10 MB · processamento em segundo plano</Text></LinearGradient></Pressable>
    <View style={styles.metrics}><Metric Icon={CalendarDays} iconColor="#62D4C7" label="PRÓXIMO PRAZO" value={deadlineText(nearest)} hint={nearest?.description || 'Nenhum prazo próximo'} /><Metric Icon={Sparkles} iconColor="#24D6AF" label="USO DISPONÍVEL" value={`${data.usage.restante}`} hint="análises restantes hoje" /></View>
