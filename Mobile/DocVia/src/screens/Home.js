...
const documentColors = {
  contract: { color: '#39C6CB', background: '#39C6CB1F', Icon: FileText },
...
export default function Home({ user, navigate, openDocument }) {
  ...
  return <ScrollView style={common.screen} contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} tintColor={violet} />}>
    ...
    <Pressable accessibilityRole="button" onPress={() => navigate('upload')} style={({ pressed }) => [styles.analyze, pressed && styles.analyzePressed]}><LinearGradient colors={['#0B4E58', '#107783', '#19A5A7']} locations={[0, .54, 1]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.analyzeGradient}><Sparkles size={24} color="#D8FBFC" strokeWidth={1.8} /><View style={styles.uploadCircle}><Upload size={17} color="#FFFFFF" strokeWidth={2} /></View><Text style={styles.analyzeTitle}>Analisar novo documento</Text><Text style={styles.analyzeText}>PDF, JPG ou PNG · até 10 MB · processamento em segundo plano</Text></LinearGradient></Pressable>
    <View style={styles.metrics}><Metric Icon={CalendarDays} iconColor="#39C6CB" label="PRÓXIMO PRAZO" value={deadlineText(nearest)} hint={nearest?.description || 'Nenhum prazo próximo'} /><Metric Icon={Sparkles} iconColor="#24D6AF" label="USO DISPONÍVEL" value={`${data.usage.restante}`} hint="análises restantes hoje" /></View>
...
const styles = StyleSheet.create({
  content: { paddingHorizontal: 22, paddingTop: 38, paddingBottom: 130, gap: 16 }, loading: { padding: 22, gap: 16 }, header: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }, welcome: { color: '#78979C', fontSize: 12, marginBottom: 6 }, greeting: { color: '#F0FAFB', fontSize: 23, letterSpacing: .1, fontWeight: '800' }, hand: { fontSize: 21 }, notification: { width: 42, height: 42, borderRadius: 21, borderWidth: 1, borderColor: '#24434A', backgroundColor: '#102126', alignItems: 'center', justifyContent: 'center' }, notificationPressed: { opacity: .8, transform: [{ scale: .96 }] }, analyze: { minHeight: 135, borderRadius: 18, overflow: 'hidden' }, analyzeGradient: { flex: 1, minHeight: 135, padding: 20 }, analyzePressed: { opacity: .87, transform: [{ scale: .99 }] }, uploadCircle: { position: 'absolute', right: 18, top: 49, width: 36, height: 36, borderRadius: 18, backgroundColor: '#FFFFFF1A', alignItems: 'center', justifyContent: 'center' }, analyzeTitle: { color: '#FFFFFF', fontSize: 16, fontWeight: '800', marginTop: 13 }, analyzeText: { color: '#D8FBFC', fontSize: 12, lineHeight: 17, marginTop: 5, maxWidth: 245 }, metrics: { flexDirection: 'row', gap: 12 }, metric: { flex: 1, minHeight: 131, borderRadius: 17, borderWidth: 1, borderColor: '#24434A', backgroundColor: '#0B171A', padding: 15 }, metricLabel: { color: '#78979C', fontSize: 8, letterSpacing: 1.3, fontWeight: '800', marginTop: 13 }, metricValue: { color: '#F0FAFB', fontSize: 16, fontWeight: '800', marginTop: 7 }, metricHint: { color: '#8FAAAF', fontSize: 10, lineHeight: 14, marginTop: 5 }, sectionRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: 12 }, sectionTitle: { color: '#E3F1F3', fontSize: 14, fontWeight: '800' }, link: { color: '#76D9DC', fontSize: 11, fontWeight: '700' }, documents: { gap: 10 }, document: { minHeight: 71, borderRadius: 17, borderWidth: 1, borderColor: '#24434A', backgroundColor: '#0B171A', paddingHorizontal: 14, flexDirection: 'row', alignItems: 'center', gap: 12 }, documentPressed: { opacity: .82, transform: [{ scale: .99 }] }, documentIcon: { width: 42, height: 42, borderRadius: 21, alignItems: 'center', justifyContent: 'center' }, documentInfo: { flex: 1, minWidth: 0 }, documentName: { color: '#F0FAFB', fontSize: 13, fontWeight: '700' }, documentMeta: { flexDirection: 'row', alignItems: 'center', marginTop: 5 }, documentType: { fontSize: 10, letterSpacing: .7, fontWeight: '900' }, documentDate: { color: '#747786', fontSize: 10, marginLeft: 6 }, emptyRecent: { minHeight: 86, borderRadius: 17, borderWidth: 1, borderColor: '#24434A', backgroundColor: '#0B171A', paddingHorizontal: 14, flexDirection: 'row', alignItems: 'center', gap: 12 }, emptyIcon: { width: 42, height: 42, borderRadius: 21, backgroundColor: '#168B941F', alignItems: 'center', justifyContent: 'center' }, emptyCopy: { flex: 1 }, emptyTitle: { color: '#F0FAFB', fontSize: 13, fontWeight: '700' }, emptyText: { color: '#8FAAAF', fontSize: 10, marginTop: 5 }
});
