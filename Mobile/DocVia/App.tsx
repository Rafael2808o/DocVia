...
import { Login, Register, Forgot, Reset } from './src/screens/Auth';
...
export default function AppRoot() {
  const [session, setSession] = useState(); const [user, setUser] = useState(); const [screen, setScreen] = useState('boot'); const [detail, setDetail] = useState(); const [resetToken, setResetToken] = useState('');
  useEffect(() => subscribeSessionExpiration(() => { setSession(); setUser(); setScreen('login'); }), []);
  useEffect(() => { (async () => { const saved = await loadSession(); if (!saved) { setScreen(await hasCompletedOnboarding() ? 'login' : 'onboarding'); return; } try { const account = await userApi.me(); const next = { ...saved, user: account }; await saveSession(next); setUser(account); setSession(next); setScreen('home'); } catch (error) { if (error?.status === 401 || error?.status === 403) { await clearSession(); setScreen('login'); return; } setUser(saved.user); setSession(saved); setScreen('home'); } })(); }, []);
  useEffect(() => {
    const openResetLink = ({ url }) => {
      if (!url) return;
      const parsed = Linking.parse(url);
    if ((parsed.path === 'reset-password' || parsed.hostname === 'reset-password') && typeof parsed.queryParams?.token === 'string') {
      setResetToken(parsed.queryParams.token);
      setScreen('reset');
    }
    };
    Linking.getInitialURL().then((url) => openResetLink({ url }));
    const subscription = Linking.addEventListener('url', openResetLink);
  ...
  */
  ...
  return <SafeAreaProvider style={styles.app}><StatusBar style="light" />{content}{session && !['detail', 'privacy', 'reset'].includes(screen) && <FloatingNav active={screen} navigate={setScreen} />}<Toast /></SafeAreaProvider>;
}
